...
> **Live map demo:** *(link to be added after GitHub push → Streamlit Cloud deploy)*  
> **3-minute walkthrough video:** *(recording in progress — link to be added before submission)*

> **Status:** runs end-to-end on the official ADB data — **Maharashtra (14,082 sections)** and **Thailand (55,884 sections)**, same code, zero changes. 791 unsafe and 1,242 non-credible limits in Maharashtra; 39,519 unsafe in Thailand; est. 1,636 lives saved/year across both regions; see §8.0.
...
2. [Methodology — The Safe System Core](#2-methodology--the-safe-system-core)
3. [Data Used](#3-data-used)
4. [How to Run](#4-how-to-run)
5. [Pipeline — 8 Stages](#5-pipeline--8-stages)
6. [System Architecture — Deterministic Core, Agentic Shell](#6-system-architecture)
7. [Outputs](#7-outputs)
8. [Validation Results](#8-validation-results)
9. [Scalability — Zero-Local-Data Mode](#9-scalability--zero-local-data-mode)
10. [Policy Integration Pathway](#10-policy-integration-pathway)
11. [Lives-Saved-per-Dollar Simulator](#11-lives-saved-per-dollar-simulator)
12. [Limitations & Anticipated Questions](#12-limitations--anticipated-questions)
...
## 2. Methodology — The Safe System Core
...
Judges can run the full system in demo mode with **no ADB data and no credentials** — a bundled open-data sample ships with the repository.

```bash
# Option A — Docker (recommended)
git clone https://github.com/ventax-ai-lab/digital-safe-system-panel
cd digital-safe-system-panel
docker compose up
...
1. **Interactive live map** — 69,966 segments colour-coded by diagnosis (red = unsafe limit, amber = non-credible, green = safe); hover for diagnosis card with three speeds (S_safe / Posted / P85), fired rule, confidence grade, and lives-saved estimate. Run locally: `streamlit run app.py` — or deploy from GitHub to Streamlit Cloud (free, 2 min).
...
2. **Per-segment policy briefs** — plain language (English + Urdu), citation-backed, critic-validated. Six sample briefs on real ADB data are in `docs/policy_briefs/`. To generate more: `python tools/run_panel.py --top 10` (requires `ANTHROPIC_API_KEY`).
...
> *"The Malegaon Bypass is the highest-priority segment identified in Maharashtra: despite only 954 daily vehicles, the compound of a P85 of 76.9 km/h against an S_safe of 30 km/h and Maharashtra's near-zero motorcycle pillion helmet compliance (PassengerSPI = 0.01 per ADB GPKG) produces an estimated 0.515 lives saved per year — the single largest per-segment return in the regional analysis. Maharashtra's highway authority should treat this segment as the pilot intervention for Safe System implementation in the Nashik district."*
...
¹ High-priority = score ≥ 90th-percentile threshold of the **combined** 69,966-segment network (threshold = 15.0, written to `network_summary.json`). Per-city counts at this threshold: Maharashtra 365, Thailand 7,401. Judges can verify: `cat docs/network_summary.json`.
...
The hardest question for any road-safety tool in our region: *what about the next city, where there is no curated dataset?*
...
- **Pakistan (worked example):** speed limits on national highways are notified by the National Highway Authority / National Highways & Motorway Police; provincial and district roads fall under provincial transport departments and district administrations (s.144 CrPC notifications are a common instrument for local limits). Our per-segment policy brief is formatted as the technical annex such a notification requires: evidence, survivability rationale, expected casualty reduction, and cost class of the intervention.
...
Full bibliographic details and DOIs are being compiled in `docs/references.bib` (Phase 6 — to be committed before submission deadline).
